...
class Category(Base):
    ...
    name:Mapped[str] = mapped_column(String(50),unique=True,comment="分类名",nullable=False)
    sort_order:Mapped[int] = mapped_column(Integer,nullable=False,default=0,comment="排序")
    def __repr__(self):
        return f"Category(id={self.name}, name={self.name}, sort_order={self.sort_order})"
